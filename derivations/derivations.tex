...
\paragraph{When the simple sum is exact.} The code uses $\kappa_e=\sum_\sigma\kappa_\sigma$, i.e.\
it drops the bracket. In a half-metal the minority channel is gapped near $E_F$, so
...
omitted term is exactly zero at the operating point and the reported $ZT$ is unaffected. The
bracket becomes relevant only for $\mu$ pushed far enough to make both spins conduct
($\mu-E_F\gtrsim+1$~eV), where it would \emph{reduce} $ZT$; it should be restored if that regime
is ever analysed.
...
and $\kappa_e=\sum\kappa_\sigma+$ bipolar term---the code drops the bipolar term, exact here
because the minority channel is gapped at the operating point ($\S5$).
\item[(vii)] $\kappa_0=\pi^2\kB^2 T/3h$ ($\S6$).
\end{itemize}
Items (ii), (iv), (v) were cross-checked numerically (boxes above); (vi)'s dropped term is the one
approximation, and is exactly zero where $ZT$ is evaluated.
