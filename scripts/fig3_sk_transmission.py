...
def main():
    ...
    fig.suptitle("Spin-resolved Landauer transmission — fitted SK model", fontsize=11)
    fig.tight_layout()
    ...
    if zero.size:
        print(f"zigzag minority zero-T window: [{zero.min():+.2f}, {zero.max():+.2f}] eV "
              f"(>= {zero.max() - zero.min():.1f} eV); majority T(E_F)="
